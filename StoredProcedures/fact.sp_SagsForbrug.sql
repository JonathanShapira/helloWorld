CREATE   PROC [fact].[sp_SagsForbrug] AS
BEGIN
    DROP TABLE IF EXISTS switch.fact_SagsForbrug
    SELECT 
         Entry_No
    ...
    where sp.Entry_Type = 'Usage'


/*
DROP TABLE IF EXISTS fact.SagsForbrug
SELECT * into fact.SagsForbrug FROM switch.fact_SagsForbrug 
*/

    EXEC utility.SwitchTableProcedure 
    @Switch_Schema = 'switch'
    ,@switch_table_name='fact_SagsForbrug'
    ,@Target_Schema='fact'
    ,@Target_Table_Name = 'SagsForbrug'
    ,@force_switch = 1
    ,@accepted_pct = 0
